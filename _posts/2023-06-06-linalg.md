...
Let's consider spaces defined by these polynomials on the plane $\mathbb{R}^2$. A line $L$ with slope $1$ passing through the origin $(0, 0)$ on the plane has the roots  of the polynomial $x - y = 0$ as points.
...
So to sum it all up, here's the key idea: let $V(f)$ be a variety. A point $(a, b) \in \mathbb{R}^2$ with evaluation map $p$ is contained in $S$ if and only if $p(f) = 0$. This automatically means that any multiple of $f$ – any polynomial in the variety $(f)$ – is contained in $S$ since $p(gf) = gp(f) = g(0) = 0$.
...
> Finally, for varieties $V(f_1, \dots, f_n)$ defined by multiple polynomials, we force any polynomials $g, h$ whose difference $g - h$ is in the ideal $(f_1, \dots, f_n)$ to be equal to get the set of unique polynomials $\mathbb{R}[x, y]/(f_1, \dots, f_n)$. That is because $g - h$ by definition would evaluate to $0$ on the ideal $(f_1, \dots, f_n)$, so $g$ and $h$ would be the same on $V(f_1, \dots, f_n)$.
...
(notice that each defining polynomial $\varphi(y_i)$ is represented by *column* in this matrix!)
...
See the similarity to $\varphi$? To evaluate the polynomial $x$ on $(a', b')$ using $\ev_{(a, b)}$, we have to take into account that the value $a$ is now $a' = a \cos(\theta) - b \sin(\theta)$. But if we use $\ev_{(a, b)} \circ \varphi$ to evaluate $x$ instead, we're all good:
