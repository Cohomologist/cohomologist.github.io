...
> Finally, this also generalizes to varieties $V(f_1, \dots, f_n)$ defined by multiple polynomials. We force any polynomials $g, h$ whose difference $g - h$ is in the ideal $(f_1, \dots, f_n)$ to be equal to get the set of unique polynomials $\mathbb{R}[x, y]/(f_1, \dots, f_n)$. That is because $g - h$ by definition would evaluate to $0$ on the ideal $(f_1, \dots, f_n)$, so $g$ and $h$ would be the same on $V(f_1, \dots, f_n)$.
